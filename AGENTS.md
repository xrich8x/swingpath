...
Orientation for Codex. Read this file; read the others **only when the table
...
| --- | --- | --- |
| Any code (`backend/`, `tools/`, `frontend/src/`, `mobile/`, `ball_physics/`) | **`docs/STATE.md`** — the number it moved, or the negative and why | `.Codex/hooks/state-guard.sh` (`[no-state]` opts out) |
| `run.py`'s argument parser | `README.md` / `USER_GUIDE.md` / `SETUP_PROMPT.md` — whichever now lies | `.Codex/hooks/docs-guard.sh` (`[no-docs]` opts out) |
...
Five teammates in `.Codex/agents/`, each with memory in `.Codex/agent-memory/<name>/`.
...
**THREE LIVE AGENTS PROJECT-WIDE — `.Codex/hooks/agent-cap.sh` counts the whole tree**, so a
...
**`.Codex/journals/` — one per agent plus `lead.md`, written DURING work, not after**; a kill
