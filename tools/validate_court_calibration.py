...
camera's view of a regulation court on THIS clip, before you trust it downstream:
    ...
    backend/.venv/Scripts/python.exe tools/validate_court_calibration.py \
        --keypoints court_pts.json --video clip.mp4

(Sibling tool: tools/validate_new_clip.py is the CLIP intake gate — resolution +
a cheap corner-orientation audit over many files at once. This one is the deep
single-calibration check: it fits the actual camera instead of assuming a FOV.)
