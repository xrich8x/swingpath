...
def _scan(axes, calibration, court, court_pts, dt, cos2, sin2, w, h, tol, athr):
    """Score every trapezoid in the axis grid. Returns [(score, nlines, params)]."""
    out = []
    for cx in axes[0]:
        for yn in axes[1]:
            for yf in axes[2]:
                ...
                for wn in axes[3]:
                    for wf in axes[4]:
                        ...
                        g, nl, _ = _ori_detail(H, calibration, court, dt, cos2, sin2,
                                               w, h, tol, athr)
                        if g > 0:
                            out.append((g, nl, (cx, yn, yf, wn, wf)))
    out.sort(key=lambda t: t[0], reverse=True)
    return out


def autodetect(frame, calibration, court, *, grid=4, topk=10,
               athr=0.80, accept=0.45):
    """Coarse grid -> local refine -> snap -> structural gate. Returns
    ...
    ranked = _scan(ax, calibration, court, court_pts, dt, cos2, sin2, w, h, tol, athr)

    # coarse-to-fine: local grid around the top-3 coarse seeds for a tighter start
    steps = [(coarse[i][1]-coarse[i][0]) * (w if i in (0, 3, 4) else h) for i in range(5)]
    seeds = []
    for _g, _nl, p in ranked[:3]:
        local = tuple(np.array([p[i]-steps[i]/2, p[i], p[i]+steps[i]/2]) for i in range(5))
        seeds += _scan(local, calibration, court, court_pts, dt, cos2, sin2, w, h, tol, athr)
    seeds += ranked
    seeds.sort(key=lambda t: t[0], reverse=True)

    best = None
    tried = 0
    for _g, _nl, p in seeds:
        if tried >= topk:
            break
        tried += 1
        try:
        ...
        # structural accept gate: good overall support, enough DISTINCT court lines
        # (>=half the in-frame lines, min 5), and the coverage/centrality self-check.
        need = max(5, int(0.5 * nseen))
        if g >= accept and nl >= need and calibration.verify_court(frame, Hs).ok \
                and (best is None or g > best[1]):
            best = (Hs, g, ref)
    return best


def score_clip(clip, per_clip, grid, topk):
    ...
    for k in usable:
        ...
        if img is None:
            continue
        res = autodetect(img, calibration, court, grid=grid, topk=topk)
        if res is None:
            continue
        det += 1
        H = res[0]
