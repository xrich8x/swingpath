"""Combined demo: players (YOLO-pose) + ball (TrackNet) + events on a real clip,
drawn into one annotated video and sample stills.

    python track_match_demo.py ../data/tennis_sample.mp4

Ball is run every frame; pose every Nth frame (CPU budget) and re-drawn between.
"""

from __future__ import annotations

import sys
import time

import cv2
import numpy as np

from swingvision import events, pose
from swingvision.ball import BallDetector, smooth_and_fill

SPLIT_Y, CENTER_X = 500, 955
COURT_POLY = [[380, 185], [1470, 190], [1850, 960], [120, 960]]
NEAR_COLOR, FAR_COLOR, BALL_COLOR = (255, 230, 0), (255, 0, 255), (0, 255, 255)
POSE_EVERY = 3


def draw_pose(frame, p, color, label):
    for a, b in pose.COCO_SKELETON:
        xa, ya, ca = p.keypoints[a]
        xb, yb, cb = p.keypoints[b]
        if ca > 0.3 and cb > 0.3:
            cv2.line(frame, (int(xa), int(ya)), (int(xb), int(yb)), color, 2, cv2.LINE_AA)
    x1, y1, x2, y2 = (int(v) for v in p.box)
    cv2.rectangle(frame, (x1, y1), (x2, y2), color, 1)
    cv2.putText(frame, label, (x1, y1 - 6), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2, cv2.LINE_AA)


def main(video_path: str):
    bd = BallDetector("weights/tracknet.pt")
    pe = pose.PoseEstimator(weights="yolo11x-pose.pt", conf=0.20, imgsz=1920)
    cap = cv2.VideoCapture(video_path)
    fps = cap.get(cv2.CAP_PROP_FPS) or 30.0
    w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))

    frames, raw_ball, poses_by_frame = [], [], []
    last_players = []
    t0 = time.time()
    while True:
        ok, frame = cap.read()
        if not ok:
            break
        i = len(frames)
        raw_ball.append(bd.detect(frame))
        if i % POSE_EVERY == 0:
            last_players = pose.select_two_players(pe.estimate(frame), SPLIT_Y, CENTER_X, COURT_POLY)
        poses_by_frame.append(list(last_players))
        frames.append(frame)
    cap.release()
    print(f"inference done: {len(frames)} frames in {time.time()-t0:.0f}s; "
          f"ball {sum(p is not None for p in raw_ball)}/{len(frames)}")

    smoothed = smooth_and_fill(raw_ball, window=7, polyorder=2)
    track = [(i / fps, float(smoothed[i, 0]), float(smoothed[i, 1])) for i in range(len(smoothed))]
    hit_idx = set(events.detect_hits(track, angle_thresh_deg=70, min_gap_s=0.25))

    near_trail, far_trail = [], []
    writer = cv2.VideoWriter(
        "../data/output/match_tracked.mp4", cv2.VideoWriter_fourcc(*"mp4v"), fps, (w, h)
    )
    saved = []
    for i, frame in enumerate(frames):
        for p in poses_by_frame[i]:
            fx, fy = p.feet()
            if fy >= SPLIT_Y:
                near_trail.append((int(fx), int(fy)))
                draw_pose(frame, p, NEAR_COLOR, "near")
            else:
                far_trail.append((int(fx), int(fy)))
                draw_pose(frame, p, FAR_COLOR, "far")
        for trail, color in ((near_trail, NEAR_COLOR), (far_trail, FAR_COLOR)):
            for k in range(1, len(trail)):
                cv2.line(frame, trail[k - 1], trail[k], color, 2, cv2.LINE_AA)
        for k in range(max(1, i - 12), i + 1):
            cv2.line(frame, (int(smoothed[k - 1, 0]), int(smoothed[k - 1, 1])),
                     (int(smoothed[k, 0]), int(smoothed[k, 1])), BALL_COLOR, 2, cv2.LINE_AA)
        bx, by = int(smoothed[i, 0]), int(smoothed[i, 1])
        cv2.circle(frame, (bx, by), 7, BALL_COLOR, 2, cv2.LINE_AA)
        if i in hit_idx:
            cv2.circle(frame, (bx, by), 16, (60, 60, 255), 3, cv2.LINE_AA)
            cv2.putText(frame, "HIT", (bx + 18, by), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (60, 60, 255), 2)
        cv2.putText(frame, "players + ball + events", (20, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        writer.write(frame)
        if i in (30, 60, 90, 115):
            fn = f"../data/output/match_frame_{i:03d}.png"
            cv2.imwrite(fn, frame)
            saved.append(fn)
    writer.release()
    print("saved:", saved)


if __name__ == "__main__":
    main(sys.argv[1] if len(sys.argv) > 1 else "../data/tennis_sample.mp4")
