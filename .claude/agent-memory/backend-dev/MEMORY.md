...
- [Chain-gate mechanism findings](chain-gate-mechanism-findings.md) — SEVEN failed attempts on the smoother; run-length does not separate, M1's coast bar fails at 19.9 px, and the resets are finally counted
- [Smoother noise model is over-dispersed](smoother-noise-model-is-over-dispersed.md) — median d2 is 12x BELOW chi2-2, P supplies 70-81% of S (not R), and the accept radius is 33-64 px not ~19
