...
| **`smooth_forecast` ADDS ghosts on every clip and removes none** - measured 2026-08-29 | **A TrackNet re-run, to bound it.** Across all 7 calibrated gold clips it adds **+12 ghost fires and removes 0**, for a **mean recall change of -1.0 pt** - up on 3 clips, down on 4, so the sign is not stable. `am_hard_utr` is worst: **1 -> 6 fires while recall falls 1.2 pts**, 4 of the 5 added being FADED, which the tool's own note says removes nothing. So the stage is not buying precision - it loses on both axes per-frame. **It does NOT explain the -12.0 pt speed-coverage cost**, a different metric (`seen_frac` over hit->landing spans, where interpolated frames may not count); both numbers stand. **An n=1 read was backwards** - on `yt_rally2` alone it looks clearly recall-positive at +4.3. Measured with `ballnet_v21`, while v1 is a founder decision for TrackNet, so the pairing is not the shipped one. | [evidence/smooth-forecast-adds-ghosts.md](evidence/smooth-forecast-adds-ghosts.md) |
