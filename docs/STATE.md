...
|---|---|---|
| **Far player: motion+contrast vs. detection - PARTIALLY TESTED** | A 15-frame, zero-new-labelling re-use of P0-3's `yt_match40` population (pre-registered, spec in the evidence file), with a mandatory random-blob null control - at a median ~9 candidate blobs per frame, "some blob is close" is near-guaranteed by chance. The AGGREGATE version of this primitive (`feet_in_court`) is already **DEAD with a BACKWARDS SIGN** at n=216 locks / 30 clips; the per-frame single-blob identity question was never run | [evidence/far-player-motion-contrast-hypothesis.md](evidence/far-player-motion-contrast-hypothesis.md) |
