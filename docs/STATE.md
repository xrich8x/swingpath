...
| **Joint line-to-model correspondence - GATE PRE-REGISTERED 2026-08-27; STEP 1 MEASURED 2026-08-29** | Nothing. Unblocked, and **step 1 works**: grouping the detected lines by CONCURRENCY (a shared vanishing point) rather than by direction separates the court's two world-parallel families on **7 of 8 held-out shell clips** and 21/30 on the tuning pool - the pool where the old angular split put 24 of 26 lines in one family. The step's own bar FAILED twice first (15/38, then 19/38): the binding constraint was an inlier tolerance guessed at 0.7 deg against lines carrying several degrees of error. C1-C6 are UNTOUCHED - this is the primitive, not the gate. Next is the joint part: cross-ratio screening of pencil members against the model's known line positions. The one build Session P named and did **not** test. All five failed branches assumed the line-to-model assignment can be settled BEFORE the homography; this solves both together. Bars: zero wrong accepts, gold **12/20 -> >=15**, no accuracy loss on the 12 that work, and the chosen correspondence shown rather than inferred. Carries a stopping rule. | [evidence/court-correspondence-gate.md](evidence/court-correspondence-gate.md) |
