...
| **A second bounce HYPOTHESIS in the smoother** (`bounce_hypothesis`) | **GATE FAILS on P2 and P4.** Recall PASSES - pooled 44.5% -> 46.2% (+9 hits on 532 clicks), up on all 3 clips, `wrong` down 1 - but ghosts rise +1 on two clips and the separation ratio is **4.50:1 against a >7 bar**, i.e. worse than the structural exchange rate it was built to beat. Fourth failure on this stage. Off by default. | [evidence/bounce-hypothesis.md](evidence/bounce-hypothesis.md) |
...
| **The ball-chain stopping rule — power fix IN FLIGHT** | **Building the 7 missing gold perception caches** (`tools/build_gold_caches.py`, ~104k frames on the GPU). Every chain measurement this project has made used the **3** clips that had caches — **74 of the gold set's 308 no-ball frames** — which is why the `bounce_hypothesis` verdict rests on a denominator of 2. The caches are reusable: every future chain or detector A/B inherits the power. | [evidence/ball-chain-gate.md](evidence/ball-chain-gate.md) |
...
| **Phone app shell** | App development, not ML. No phone fps has ever been measured, so do not quote one | - |
