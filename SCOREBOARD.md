...
| **9 solid ghost balls — of which FIVE are universal** | The count is stable (v21 pooled 9, two 15-epoch arms 14 and 15, 9 shared between the arms) but the *composition* is only about half shared. **Five frames defeat all three models tested**: yt_rally2 18/762/1494, yt_match40 4773, am_hard_utr 13276. Look at those before training anything — the next detector idea (RacketVision 5-keypoint racket pose, MIT, weights released; must *condition* the detector, not filter it, per its own degradation finding) should be aimed at what is actually in them. **Read Trap 9 first**: at 74 no-ball frames the gate can only see a near-elimination. |
