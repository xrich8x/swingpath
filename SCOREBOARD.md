...
| **The rally/score layer has GROUND TRUTH** (`tools/score_truth.py`) | **The last unmeasured layer in the product is measured.** Ball had 1,851 human clicks, court 20 clips, speed the HUD and synth_truth; rallies and score had **nothing**, so "63 rallies is wrong" was an assertion. Read from each clip's own burned-in broadcast scoreboard — independent of anything this project computes — by clustering each FIELD into distinct visual states and labelling every state **by eye**. Result: **yt_match40 = 43 points** (6 game changes, 5–1) against the committed pipeline's **63 rallies** — **over-split 1.47×**; **am_hard_utr = 36 points** (2–4). **Every one of the 79 point transitions across both clips is legal tennis**, which is the independent check a misread would fail. Per-point winner resolved for **43 of 43** on yt_match40. **CORRECTS the 2026-08-13 research**, which estimated "≈35–40 points, ≈1.6×" from whole-PANEL pixel-states: truth is 43, *outside* that range, because a whole-panel state also changes when the server dot moves — per-field clustering separates points, games and serve exactly. Two saves that automation would have missed: the clustering **false-split an empty dot box in two** (would have invented a serve change), and yt_match40's state #0 is **the board not being displayed** (would have invented two points). `gap_s` deliberately **untouched** — the reference comes first. Evidence: data/output/score_truth.md | 2026-08-15 |
