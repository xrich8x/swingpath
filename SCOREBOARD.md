...
| **Whether the +5.6 pt recall gain reaches the product** | **The one thing standing between this and a shipped detector.** `weights/pool_new_s0.pt` finds materially more ball than anything measured here, but that is a *detector* number: the chain test (`tools/eval_model_filters.py` on the calibrated clips — chain recall and SOLID ghosts) has not been run, and it costs a fresh perception pass per clip. Two reasons not to promote it first: the shipped `ballnet_v21` scores **9** solid ghosts while Session I's 15-epoch arms scored **14–15** despite better detector recall, and arm B is also a 15-epoch run; and the "detector precision does not reach the product" finding below is about *precision*, so it neither predicts nor excuses what a *recall* gain does downstream. Note the recall gain could plausibly make ghosting **worse** — more firing is more firing. |
