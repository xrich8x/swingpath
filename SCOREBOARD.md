...
| **The ball training set has no backup, and cannot be rebuilt** | `data/ball_dataset/` is **43,904 frames / 2.0 GB**, gitignored and tracked by nothing, and **every one of the 14 clips' source videos is gone** — only 512×288 extracted JPEGs survive. Losing this disk loses 26,293 labelled frames outright. Partial recovery only: 10 of 14 record an 11-char YouTube id in their provenance, so those *might* re-download, but re-processing yields different pseudo-labels and videos get taken down. Needs an off-machine copy (not git — 2 GB); this is a decision, not a task. |
