...
| **Whether the +5.6 pt recall gain reaches the product** | **The one thing standing between this and a shipped detector, and looking at the errors made the case stronger.** The A/B was arm-vs-arm; against the **shipped** `ballnet_v21` the picture is a real trade, not a free win: v21 fires on **102** of the 308 no-ball frames, arm B on **166**. Arm B fires on **76 frames v21 does not** and avoids only **12** that v21 hits, and the 76 are spread across all ten clips (3–14 each), not concentrated in one venue. So +12 pts of recall over shipped comes with +20.8 pts of per-frame false fire over shipped. The chain test (`tools/eval_model_filters.py` on the calibrated clips — chain recall and SOLID ghosts) still has not been run and costs a fresh perception pass per clip; it is now the deciding measurement, because per-frame false fire is *not* the product (Session F) and the chain has repeatedly absorbed detector-level precision differences. Evidence: data/output/false_fires/, viewer at false_fires.html. |
...
| **Rally segmentation over-splits — and it makes the SCORE wrong** | **Re-verified 2026-08-13 on yt_match40 from the committed perception cache**, and it is worse than first recorded. 63 rallies over 5.9 min / 196 shots. Median inter-rally gap **0.00 s**; **60% of gaps are ≤0.05 s** (literally contiguous); **32% of "rallies" hold a single shot**; and — the decisive one — **0 of 62 gaps are ≥10 s**, so in six minutes of tennis the segmenter never once found a real between-point break (real tennis is ~20–25 s between points). This is not cosmetic: `scoring` consumes one rally as one point, so the dashboard's score is derived from ~63 "points" in a stretch that really holds 8–15. Highlights inherit it too — a reel of 63 clips, one every 5.6 s. **A LOGIC-layer bug, so it is exactly fixable** — no GPU, no labels, no model. Evidence: data/output/rallycheck.json |
