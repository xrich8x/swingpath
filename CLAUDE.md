...
<!-- MAINTAINER NOTE: this file is auto-loaded every session and every line costs
     context on every turn. Hard cap: 150 lines. If you are about to add a finding,
     a number, or a session entry, it does NOT go here — see the doc map. Block-level
     HTML comments are stripped before this file reaches context, so notes like this
     one are free. -->
...
**Offline-first**: record, then process. No real-time requirement.

`schema.py` is the only contract between the halves. Either side may change freely
as long as it holds.
...
11. **Truth comes from the GAME, not the VIDEO.** The court, the ball, the players,
    the bounces, the physics — never a scoreboard, HUD or graphic burned into the
    frame. That is somebody's data entry *about* the game, and it is barred as a
    training target, a ground-truth reference AND a tuning signal. Independence is
    not truth: a diligently-kept WRONG board is perfectly self-consistent, and
    nothing leaning on an overlay generalises to the user's own phone clip.
    Compliant references: human clicks, `tools/synth_truth.py`, geometry we derive.
    **One live exception, flagged not hidden:** `tools/hud_ocr.py` reads
    SwingVision's MPH panel; figures citing a "HUD MAE" are *agreement with another
    estimator, not accuracy*, and must be labelled that way. Add no new ones.
12. **The rally / score layer is OUT OF SCOPE** (user ruling, 2026-08-20). Not an
    open problem, not a backlog item. Do not propose work on point boundaries,
    rally segmentation, the `gap_s` override, the second-bounce rule, or any
    ground-truth source for points. Leave `scoring.py` / `corrections.py` alone
    rather than removing them — the corrections replay depends on them, and
    `stats.score_validation_note` is what stops the UI presenting a scoreline as a
    measurement.
...
- Court constants: `backend/swingvision/court.py`, mirrored in
  `frontend/src/lib/court.js`. Call-accuracy table mirrored in `calls.js`. Both
...
- Every pixel threshold scales by `frame_height/720`. Exception: `static_radius_px`,
...
A STATE entry is **one line**: what changed, the number, the evidence path. Prose
goes in the evidence file. If your row needs a paragraph, you are writing in the
wrong file.
...
- **Audit every calibration before trusting it.** Some committed `data/*_pts.json`
  are degenerate and break the overlay + ball gating silently.
  `tools/validate_new_clip.py --audit <files>` — good is < 2.5 px fit residual.
  Details and the known-good/known-bad list: `docs/calibration.md`.
...
- **`demo30` is 0.5 px but low (1.38 m)** — measures 5.2 m of 23.77. Do not cite
  its speeds.
